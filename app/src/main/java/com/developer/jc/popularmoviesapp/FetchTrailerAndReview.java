...
public class FetchTrailerAndReview extends AsyncTask<String, Void, Void> {
    ...
    public FetchTrailerAndReview(Context context, GridView gridview){
        mContext = context;
        mGridView = gridview;
    }
     ...
     */
    public void parseMovieReview(JSONObject json) throws JSONException {
    ...
    public void parseMovieVideo(JSONObject json) throws JSONException {
        //results array in main object
        results = json.getJSONArray("results");
        //loop through array and create 20 Movie objects
        for (int i = 0; i < results.length(); i++) {
            ...
            movie.setMovieId(currentMovie.getInt("id"));
            movie.setPosterPath(currentMovie.getString("iso_639_1"));
            movie.setOverView(currentMovie.getString("key"));
            movie.setReleaseDate(currentMovie.getString("name"));
            movie.setOriginalTitle(currentMovie.getString("site"));
            movie.setVoteAverage(currentMovie.getDouble("type"));
            movies[i] = (movie);
        }
    }
}